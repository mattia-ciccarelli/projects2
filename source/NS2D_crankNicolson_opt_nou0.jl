...
function crankNicolson_opt_nou0(input_file)

  output_dir = "./output/_timeint_true/"

  fname = "../DPIM_NS/input/" * input_file
  include(fname)
  info.mesh_file = file
  ...
  for inu = 1:nnu
    re_value = coeffnorm * char_l / nu
    timestep_str = lpad(string(0), 5, "0")
    export_timestep(nodes, B3, T6, "$(output_dir)", "/g$(g_value)_$(timestep_str)", u_n)
    ...
    for i = 1:nstep
      ...
      if ((i >= 0.95*nstep && i % floor(Int, nstepd / 50) == 0) || (i < nstepd && i % floor(Int, nstepd / 100) == 0) )
